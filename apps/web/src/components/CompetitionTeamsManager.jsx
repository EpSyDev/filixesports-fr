...
const CompetitionTeamsManager = ({ competitionId }) => {
  ...
  const fetchTeams = async () => {
    setLoading(true);
    try {
      const records = await pb.collection('league_teams').getFullList({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        const updated = await pb.collection('league_teams').update(editingId, {
          teamName: teamName.trim()
        }, { $autoCancel: false });
        setTeams(teams.map(t => t.id === editingId ? updated : t));
        toast.success('Équipe mise à jour.');
      } else {
        const created = await pb.collection('league_teams').create({
          competitionId,
          teamName: teamName.trim()
        }, { $autoCancel: false });
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-card shadow-sm border-border">
        <CardHeader>
          <CardTitle className="text-lg">{editingId ? 'Modifier l\'équipe' : 'Ajouter une équipe'}</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              teams.map(team => (
                <div key={team.id} className="flex justify-between items-center p-3 bg-muted/30 border border-border/50 rounded-lg hover:bg-muted/60 transition-colors">
                  ...
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" onClick={() => startEdit(team)} className="text-primary hover:text-primary hover:bg-primary/10">
                      <Edit2 className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(team.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
