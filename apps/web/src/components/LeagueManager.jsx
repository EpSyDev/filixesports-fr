...
const LeagueManager = ({ competition }) => {
  ...
  useEffect(() => { fetchLeagueData(); }, [competition.id]);

  const fetchLeagueData = async () => {
    setLoading(true);
    try {
      const [teamsRes, matchesRes, standingsRes] = await Promise.all([
        supabase.from('league_teams').select('*').eq('competitionId', competition.id),
        supabase.from('league_matches').select('*').eq('competitionId', competition.id).order('matchday', { ascending: true }),
        supabase.from('league_standings').select('*').eq('competitionId', competition.id).order('rank', { ascending: true })
      ]);
  ...
  const refreshStandings = async () => {
    ...
    try {
      const { data, error } = await supabase.from('league_standings').select('*').eq('competitionId', competition.id).order('rank', { ascending: true });
  ...
  const handleAddTeam = async (e) => {
    ...
    try {
      const { data: record, error } = await supabase.from('league_teams').insert({ competitionId: competition.id, teamName: newTeam.trim() }).select().single();
      if (error) throw error;
  ...
  const handleGenerateCalendar = async () => {
    if (teams.length < 2) { toast.error('Il faut au moins 2 équipes'); return; }
    setGenerating(true);
    try {
      // Supprime les matchs et classements existants
      await Promise.all([
        supabase.from('league_matches').delete().eq('competitionId', competition.id),
        supabase.from('league_standings').delete().eq('competitionId', competition.id)
      ]);

      const newMatches = generateRoundRobinMatches(teams);
      const matchesToInsert = newMatches.map(m => ({
        competitionId: competition.id, matchday: m.matchday,
        homeTeam: m.homeTeam, awayTeam: m.awayTeam, status: 'scheduled'
      }));
      const standingsToInsert = teams.map((t, i) => ({
        competitionId: competition.id, teamName: t.teamName,
        played: 0, won: 0, drawn: 0, lost: 0, points: 0, goalsFor: 0, goalsAgainst: 0, rank: i + 1
      }));

      const [matchesRes, standingsRes] = await Promise.all([
        supabase.from('league_matches').insert(matchesToInsert).select(),
        supabase.from('league_standings').insert(standingsToInsert).select()
      ]);
      if (matchesRes.error) throw matchesRes.error;
      if (standingsRes.error) throw standingsRes.error;

      setMatches(matchesRes.data.sort((a, b) => a.matchday - b.matchday));
      setStandings(standingsRes.data);
  ...
  const updateStandingsInDb = async (newMatches) => {
    ...
    await Promise.all(newStandingsData.map(ns => {
      const existing = standings.find(s => s.teamName === ns.teamName);
  ...
  const handleSaveMatchScore = async (matchId, homeScore, awayScore) => {
    ...
    try {
      const { data: saved, error } = await supabase.from('league_matches').update({ homeScore: hScore, awayScore: aScore, status }).eq('id', matchId).select().single();
      if (error) throw error;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        <div className="lg:col-span-4 space-y-6">
          <Card className="bg-card border-border shadow-sm">
            <CardHeader><CardTitle className="text-lg">Équipes Participantes ({teams.length})</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <form onSubmit={handleAddTeam} className="flex gap-2">
                <Input placeholder="Nom de l'équipe" value={newTeam} onChange={(e) => setNewTeam(e.target.value)} className="bg-background text-foreground min-h-[44px]" />
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {teams.map(team => (
                  <div key={team.id} className="flex items-center justify-between bg-muted/50 p-2.5 rounded-lg border border-border/50">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleDeleteTeam(team.id)} className="h-10 w-10 text-destructive hover:bg-destructive/10"><Trash2 className="w-5 h-5" /></Button>
                  </div>
                ))}
                {teams.length === 0 && <p className="text-center text-sm text-muted-foreground py-4">Aucune équipe ajoutée.</p>}
              </div>
              <div className="pt-4 border-t border-border">
                <Button onClick={handleGenerateCalendar} disabled={teams.length < 2 || generating} className="w-full gap-2 min-h-[44px]">
        ...
        <div className="lg:col-span-8 space-y-6">
          {standings.length > 0 && (
            <Card className="bg-card border-border shadow-md relative overflow-hidden">
              ...
              <CardHeader><CardTitle className="text-lg">Classement</CardTitle></CardHeader>
                    ...
                    <TableBody>
                      {standings.map((s, index) => (
                        <TableRow key={s.id} className={cn(index < 3 && 'bg-primary/5')}>
                          ...
                          <TableCell className="font-semibold"><ClubBadge teamName={s.teamName} /></TableCell>
          ...
          {matches.length > 0 && (
            <Card className="bg-card border-border shadow-md">
              <CardHeader><CardTitle className="text-lg">Calendrier des matchs</CardTitle></CardHeader>
              <CardContent>
                    ...
                    <div key={day} className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                        {dayMatches.map(match => (
                          <InlineMatchScoreInput key={match.id} match={match} onSave={handleSaveMatchScore} onDelete={handleDeleteMatch} />
                        ))}
                      </div>
                    </div>
                  ))}
